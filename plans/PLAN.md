...
**Active:**
- [`PLAN-emergency-only.md`](PLAN-emergency-only.md) — 🆕 **sticky "Emergency calls only" lane (opened 2026-09-15, plan only).** Distinct from the FIXED ~50 s cold wedge (`18771530`) and the ~95 s cell-edge transient: the modem parks on AT&T/T-Mobile emergency-only and STAYS until an airplane toggle (Gold 07-20/21, new2 09-14 on the road). Leading model = IMS loss → voice-centric domain selection abandons 311480 (July `nas-probe` finding), helped by the ROM default `ro.telephony.default_network=33` (GSM/WCDMA in the mask). Plan = flight recorder on new2 + deterministic bench Cell F (force IMS off under Global vs LTE-only × voice/data-centric) + episode ladder + watchdog/default-mode fix lanes.
